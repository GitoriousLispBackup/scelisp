...
(defcfun "SCE_Shader_SetParam" :void
  (index :int)
  (val :int))
(defcfun "SCE_Shader_SetParamf" :void
  (index :int)
  (val :float))
...
(defmacro with-simple-scene ((&key (width 800) (height 600)
                                   (framerate 30)
                                   (lightcolor '(1.0 1.0 1.0))
                                   (lightpos '(1.0 1.0 1.0)))
                             &body body)
  `(sdl:with-init ()
     (sdl:window ,width ,height :opengl t)
     (setf (sdl:frame-rate) ,framerate)
     (with-interface
       (with-objects ((*scene* scene)
                      (*camera* camera)
                      (*light* light :color ',lightcolor))
         (sce-camera-setviewport *camera* 0 0 ,width ,height)
         ...
         (macrolet ((update-and-render-simple-scene ()
                      `(progn
...
(defmacro with-simple-model ((path &key mesh model matrix) &body body)
  (let ((mesh (if mesh mesh (gensym)))
        (model (if model model (gensym)))
    ...
    `(with-object (,model model)
       (with-mesh (,mesh ,path)
         ...
         (let ((,matrix (sce-node-getmatrix (sce-model-getrootnode ,model)
                                            :node-read-matrix)))
           ,@body)))))